...
if (oto==false){
  ...
  delayMicroseconds(10);
  digitalWrite(trigPin1, LOW);  
  zaman = pulseIn(echoPin1, HIGH); 
  mesafe1= zaman/29.1/2;
  if(mesafe1<=25){
    dur();
    veri="z";
    }
...
digitalWrite(26,LOW);
 ...
 sag_durum = digitalRead(sag_sensor);
   ...
   if (sol_durum == HIGH && sag_durum == HIGH) // iki sensör de siyah görmüyor ise motorlar ileri gidecek şekilde çalışıtırıldı.
 {
 analogWrite(enA, 206); 
    analogWrite(enB, 206);   
   analogWrite(enA2, 206); 
    analogWrite(enB2, 206); 
    motorileri();
 }
 else if (sol_durum == HIGH && sag_durum == LOW) // sağ sensör siyah görüyor ise motorlar sağa dönecek şekilde çalıştırıldı.
 {
   analogWrite(enA, 206); 
   analogWrite(enB, 206);   
   analogWrite(enA2, 206); 
 ...
 else if (sol_durum == LOW && sag_durum == HIGH) // sol sensör siyah görüyor ise motorlar sola dönecek şekilde çalıştırıldı.
 {
   analogWrite(enA, 206); 
   analogWrite(enB, 206);   
   analogWrite(enA2, 206); 
